...
parser.add_argument("--branch", action="store", default="tree4")
FLAGS = parser.parse_args()

input_dir = os.path.join("gh_exported", FLAGS.branch)
...
def main():
    ...
    if os.path.exists(output_dir) is False:
        print(output_dir, " does not exists")
        os.mkdir(output_dir)
    ...
    for filename in filenames:
        ...
        ### skip existing files
        if(filename + "_all.npy" in outputfiles): 
        ...
        fig_path = os.path.join(output_dir , "images", filename + ".png")
